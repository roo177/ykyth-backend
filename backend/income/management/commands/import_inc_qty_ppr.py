...
from income.models import IncomeQuantitybyPPR
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Ensure df_data is a DataFrame
            ...
            # Unpivot DataFrame
            df_melted = pd.melt(df, id_vars=['rep_month', 'aygm_poz', 'tip', 'detay','PPr No.'], 
        ...
        # Save records to the database
            with transaction.atomic():
                ...
                if income_records:
                    IncomeQuantitybyPPR.objects.filter(ppr_no=ppr_no).delete()
                    IncomeQuantitybyPPR.objects.bulk_create(income_records)
                    
            self.stdout.write(self.style.SUCCESS('PPR data imported successfully'))
